...
var=$(peer chaincode query -C composerchannelrest -n TrancheInvestorMap1CC -c '{"Args":["GetTotalMonthlyPrincipalToBePaidToTrancheById","'"$trancheID"'"]}')
echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> monthlyPrincipalToBePaidToTranche.json
