...
var=$(peer chaincode query -C composerchannelrest -n TrancheInvestorMap1CC -c '{"Args":["CalculateMonthlyPrincipalForInvestor","'"$numOfUnitsPurchased"'","'"faceValue"'"]}')
echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> monthlyPrincipalForInvestor.json
