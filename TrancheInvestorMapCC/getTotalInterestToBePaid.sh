...
var=$(peer chaincode query -C composerchannelrest -n TrancheInvestorMap1CC -c '{"Args":["GetTotalMonthlyInterestToBePaidToTrancheById","'"$trancheID"'"]}')
echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> monthlyInterestToBePaidToTranche.json
