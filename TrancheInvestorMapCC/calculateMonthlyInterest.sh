...
echo "numOfUnitsPurchased is" $1
numOfUnitsPurchased=$1

echo "faceValue is" $2
faceValue=$2

echo "couponRate is" $3
couponRate=$3

var=$(peer chaincode query -C composerchannelrest -n TrancheInvestorMap1CC -c '{"Args":["CalculateMonthlyInterestForInvestor","'"$numOfUnitsPurchased"'","'"faceValue"'","'"couponRate"'"]}')
echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> monthlyInterestForInvestor.json
