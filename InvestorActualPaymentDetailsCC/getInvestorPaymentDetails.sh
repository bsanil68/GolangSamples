...
if [ -e investorPaymentDetails.json ]
then
   rm -rf investorPaymentDetails.json
fi

var=$(peer chaincode query -C composerchannelrest -n InvestorActualPaymentDetailsinfoCC -c '{"Args":["GetInvestorPaymentDetails","'"$investorId"'"]}') 
echo $var | sed -e 's/Query\ Result\:\ //g' >> investorPaymentDetails.json
