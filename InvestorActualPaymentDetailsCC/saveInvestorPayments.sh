...
echo "investorId is " $1
investorId=$1
echo "interestPaid is " $2
interestPaid=$2
echo "monthOfInterest is " $3
monthOfInterest=$3

echo "principalPaid is " $4
principalPaid=$4

echo "monthOfPrincipal is " $5
monthOfPrincipal=$5

echo "outstandingInterest is " $6
outstandingInterest=$6

echo "outstandingPrincipal is " $7
outstandingPrincipal=$7

echo "monthPaidIn is " $8
monthPaidIn=$8

echo "overdueInterest is " $9
overdueInterest=$9

echo "overduePrincipal is " ${10}
overduePrincipal=${10}
...
peer chaincode invoke -o orderer0:7050  --tls --cafile /root/bcnetwork/conf/crypto-config/ordererOrganizations/ordererorg0/orderers/orderer0.ordererorg0/msp/tlscacerts/tlsca.ordererorg0-cert.pem -C composerchannelrest -n InvestorActualPaymentDetailsinfoCC -c '{"Args":["SaveInvestorPaymentDetails","'"$investorId"'","'"$interestPaid"'","'"$monthOfInterest"'","'"$principalPaid"'","'"$monthOfPrincipal"'","'"$outstandingInterest"'","'"$outstandingPrincipal"'","'"$monthPaidIn"'","'"$overdueInterest"'","'"$overduePrincipal"'","'"$Currency"'"]}'
