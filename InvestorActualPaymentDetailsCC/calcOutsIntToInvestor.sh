...
echo "couponRate is" $4
couponRate=$4

echo "notePeriod is" $5
notePeriod=$5

var=$(peer chaincode query -C composerchannelrest -n InvestorActualPaymentDetailsinfoCC -c '{"Args":["CalculateOutstandingInterestOfInvestor","'"$coll_amount"'","'"$numOfUnitsPur"'","'"$faceValue"'","'"$couponRate"'","'"$notePeriod"'"]}') 
echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> investorOutstandingInterest.json
