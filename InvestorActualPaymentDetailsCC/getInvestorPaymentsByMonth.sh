...
if [ -e investorPaymentDetailsByMonth.json ]
then
rm -rf investorPaymentDetailsByMonth.json
fi

var=$(peer chaincode query -C composerchannelrest -n InvestorActualPaymentDetailsinfoCC -c '{"Args":["GetInvestorPaymentDetailsByMonth","'"$month"'"]}') 
echo $var | sed -e 's/Query\ Result\:\ //g' >> investorPaymentDetailsByMonth.json
