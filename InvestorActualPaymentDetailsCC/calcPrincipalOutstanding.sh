...
echo "collection Amount is" $1
coll_amount=$1

echo "numOfUnitsPur is " $2
numOfUnitsPur=$2

echo "faceValue is" $3
faceValue=$3

echo "outstandingPrincipalOfPreviousmonth is" $4
outstandingPrincipalOfPreviousmonth=$4

var=$(peer chaincode query -C composerchannelrest -n InvestorActualPaymentDetailsinfoCC -c '{"Args":["CalculateOutstandingPrincipalOfInvestor","'"$coll_amount"'","'"$numOfUnitsPur"'","'"$faceValue"'","'"$outstandingPrincipalOfPreviousmonth"'"]}') 
echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> investorOutstandingPrincipal.json
