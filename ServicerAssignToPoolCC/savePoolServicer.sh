...
echo "poolID is " $1
poolID=$1
echo "servicerID is " $2
servicerID=$2
echo "approvedStatus is " $3
approvedStatus=$3
echo "approvedDate is " $4
approvedDate=$4

peer chaincode invoke -o orderer0:7050  --tls --cafile /root/bcnetwork/conf/crypto-config/ordererOrganizations/ordererorg0/orderers/orderer0.ordererorg0/msp/tlscacerts/tlsca.ordererorg0-cert.pem -C composerchannelrest -n ServicerAssignToPoolCC -c '{"Args":["SaveApproval","'"$poolID"'","'"$servicerID"'","'"$approvedStatus"'","'"$approvedDate"'"]}'
