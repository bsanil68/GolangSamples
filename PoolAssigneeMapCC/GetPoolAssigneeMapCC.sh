...
if [ -e /home/asrikantiah/json/assigneeDetails.json ]
then
    rm -rf /home/asrikantiah/json/assigneeDetails.json
fi
var=$(peer chaincode query -C composerchannelrest -n PoolAssigneeMapCCode --version 1.4  -c '{"Args":["GetPoolAssigneeMap","'"$PoolID"'"]}')

echo $var | sed -e 's/Query\ Result\://g' >>/home/asrikantiah/json/assigneeDetails.json
