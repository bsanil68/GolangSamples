...
echo "DocumentId is:" $1
DocumentId = $1
var = $(peer chaincode query -C composerchannelrest -n Document6cc -c '{"Args":["GetDocumentByID","'"$DocumentId"'"]}')

echo $var | sed -e 's/Query\ Result\:\ \[//g' -e 's/\]//g' >> DocumentbyId.json
