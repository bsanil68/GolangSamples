...
if [ -e DocumentDetailsByDocID.json ]
then
   rm -rf DocumentDetailsByDocID.json
fi
var=$(peer chaincode query -C composerchannelrest -n Document6cc -v 1.6  -c '{"Args":["GetDocumentByID","'"$docID"'"]}')
echo $var | sed -e 's/Query\ Result\://g' >> DocumentDetailsByDocID.json
